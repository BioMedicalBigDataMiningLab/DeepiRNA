...
ratio = par.ratio1
# File name
file_name = 'mouse_'+str(ratio)

# Init and visual model
model = data_model.lstm_mouse()

# Get data
X, y = common.get_data(par.mouse_max_length, ratio)

# Training process
print('*' * 20, 'Training start', '*' * 20)
for seed in range(2018, par.seeds):
    ...
    for i in range(par.cv):
        ...
        train_data = cv_X_train[i].reshape((-1, par.timestep,
                                            par.x_dim * par.mouse_max_length))
        ...
        history = model.fit(train_data,
                            train_label,
                            batch_size=par.mouse_batch_size,
                            epochs=par.mouse_epochs,
                            verbose=0,
                            callbacks=[data_model.early_stopping])
        # Predicting
        validation_y_pred = model.predict(validation_data, batch_size=par.mouse_batch_size)
